# MODIFIED version of https://raw.githubusercontent.com/cocodataset/cocoapi/8c9bcc3cf640524c4c20a9c40e89cb6a2f2fa0e9/PythonAPI/pycocotools/cocoeval.py
...
class COCOevalWorm:
    ...
    def __init__(self, cocoGt=None, cocoDt=None, iouType='segm'):
        ...
        self.gt_ious_overlap = {}  # ious between all gts (MODIFIED)
    ...
    def evaluate(self):
        ...
        if p.iouType == 'segm' or p.iouType == 'bbox':
            ...
            # MODIFIED
            self.gt_ious_overlap = {(imgId, catId): self.computeGTIoU(imgId, catId) \
    ...
    # MODIFIED computeIoU
    def computeGTIoU(self, imgId, catId):
        overlap_thresh_bbox = 0.25
        overlap_thresh_segm = 0.0
        overlap_thresh = None
        ...
        if p.iouType == 'segm':
            g = [g['segmentation'] for g in gt]
            overlap_thresh = overlap_thresh_segm
        elif p.iouType == 'bbox':
            g = [g['bbox'] for g in gt]
            overlap_thresh = overlap_thresh_bbox
        else:
            raise Exception('unknown iouType for iou computation')
        ...
        overlapping_gts = np.full(len(ious), False)
        overlapping_ids = np.argwhere(ious > overlap_thresh)  # find overlapping gts
        for ids in overlapping_ids:
            if ids[0] == ids[1]:
                # same gts -> IoU 1.0 -> No 'real' overlap
                continue
            overlapping_gts[ids[0]] = True
            overlapping_gts[ids[1]] = True

        return overlapping_gts
    ...
    def evaluateImg(self, imgId, catId, aRng, maxDet):
        ...
        self.gt_ious_overlap[imgId, catId]  # MODIFIED
        ...
        dtOver = np.zeros((T, D))  # MODIFIED
        dtNonOver = np.zeros((T, D))  # MODIFIED
        overlap_det = np.full((T, D), -1)  # MODIFIED
        dtIgOver = np.zeros((T, D))  # MODIFIED
        dtIgNonOver = np.zeros((T, D))  # MODIFIED
        gtIgOver = np.zeros(len(gtIg))  # MODIFIED
        gtIgNonOver = np.zeros(len(gtIg))  # MODIFIED
        ...
        if not len(ious) == 0:
            for tind, t in enumerate(p.iouThrs):
                for dind, d in enumerate(dt):
                    ...
                    overlapping = gt_ious_overlap[m]  # MODIFIED
                    overlap_det[tind, dind] = overlapping  # MODIFIED
        ...
        return {
            ...
            'gtIgnore': gtIg,
            'dtIgnore': dtIg,
            'overlapGT': gt_ious_overlap,  # MODIFIED
            'overlapDT': overlap_det,  # MODIFIED
            'dtOver': dtOver,  # MODIFIED
            'dtNonOver': dtNonOver,  # MODIFIED
            'dtIgOver': dtIgOver,  # MODIFIED
            'dtIgNonOver': dtIgNonOver,  # MODIFIED
            'gtIgOver': gtIgOver,  # MODIFIED
            'gtIgNonOver': gtIgNonOver,  # MODIFIED
        }
    ...
    def accumulate(self, p=None):
        ...
        tps_col = -np.ones((T, R, K, A, M))
        fps_col = -np.ones((T, R, K, A, M))
        ...
        tps_over_col = -np.ones((T, R, K, A, M))
        fps_over_col = -np.ones((T, R, K, A, M))
        ...
        tps_non_over_col = -np.ones((T, R, K, A, M))
        fps_non_over_col = -np.ones((T, R, K, A, M))
        ...
        for k, k0 in enumerate(k_list):
            ...
            for a, a0 in enumerate(a_list):
                ...
                for m, maxDet in enumerate(m_list):
                    ...
                    # MODIFIED
                    # overlapGT = np.concatenate([e['overlapGT'] for e in E])  # MODIFIED
                    # overlap_count = np.count_nonzero(overlapGT)  # MODIFIED
                    # non_overlap_count = np.count_nonzero(overlapGT == 0)  # MODIFIED
                    ...
                    #overlapDT = np.concatenate([e['overlapDT'][:, 0:maxDet] for e in E], axis=1)[:, inds]
                    #dt_overlap_count = np.count_nonzero(overlapDT == 1)
                    #dt_non_overlap_count = np.count_nonzero(overlapDT == 0)
                    ...
                    for t, (tp, fp) in enumerate(zip(tp_sum, fp_sum)):
                        ...
                        # MODIFIED
                        # axs[t%4, t//4].plot(p.recThrs, q)
                        # axs[t%4, t//4].set(xlabel='Recall', ylabel='Precision')
                        # axs[t%4, t//4].grid()
    ...
    def summarize(self):
        ...
        def _summarize(ap=1, iouThr=None, areaRng='all', maxDets=100, mode=0):  # MODIFIED
            p = self.params
            ...
            if ap == 1:
                ...
                raise NotImplementedError('AP mode not supported')
            ...
            if ap == 1:
                ...
                raise NotImplementedError('Method for ap {} not implemented'.format(ap))
